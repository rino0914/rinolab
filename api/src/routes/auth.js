...
export function createAuthRouter(options = {}) {
    ...
    const authenticate = options.authenticate ?? authenticateAccount;
    ...
    router.post("/login", loginLimiter, async (request, response) => {
        try {
            ...
            const account = await authenticate(normalizedEmail, password);
            ...
            request.session.authenticatedAt = Math.floor(Date.now() / 1000);
